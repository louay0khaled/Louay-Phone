...
export async function handleTelegramUpdate(update: TelegramUpdate) {
  ...
  // Never create a customer/conversation for the administrator's own chat.
  if (adminChatId !== null && chatId === adminChatId) {
    if (text.startsWith('/reply')) {
      ...
      if (conversation) {
        ...
        targetChatId = conversation.telegram_chat_id;
      }

      if (!targetChatId) {
        const { data: order } = await supabase.from('orders').select('id, customer_id').eq('id', parsed.ticketId).maybeSingle();
        if (order?.customer_id) {
          const { data: customer } = await supabase.from('customers').select('telegram_chat_id').eq('id', order.customer_id).maybeSingle();
          targetChatId = customer?.telegram_chat_id ?? null;
          if (!conversationId) {
            const { data: linkedConversation } = await supabase.from('conversations').select('id').eq('customer_id', order.customer_id).maybeSingle();
  ...
  const { data: conversation } = await supabase.from('conversations').select('id').eq('telegram_chat_id', chatId).maybeSingle();
  let conversationId = conversation?.id as string | undefined;
  if (!conversationId) {
    const { data, error } = await supabase
      .from('conversations')
      .insert({ customer_id: customerId, telegram_chat_id: chatId, status: 'open', last_message_at: new Date().toISOString() })
      .select('id')
      .single();
    if (error) throw error;
  ...
  if (adminChatId !== null) {
    await sendTelegramMessage(
      ...
      [
        ...
        `<b>التذكرة:</b> <code>${escapeHtml(conversationId)}</code>`,
        `<b>الاسم:</b> ${escapeHtml(name)}`,
        `<b>المعرف:</b> ${escapeHtml(username ?? '—')}`,
        `<b>الرسالة:</b>`,
        escapeHtml(text),
        '',
        `<b>للرد:</b> <code>/reply ${escapeHtml(conversationId)} نص الرد</code>`,
      ].join('\n'),
    );
  }
