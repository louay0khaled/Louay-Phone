...
export default function ProductGallery({ images, name }: { images: GalleryImage[]; name: string }) {
  ...
  if (!current) {
    return <div className="premium-product-gallery premium-product-gallery--empty" aria-label={`لا توجد صور لـ ${name}`}>لا توجد صور متاحة</div>;
  }
    ...
    <div className="premium-product-gallery">
      <div className="premium-product-gallery__stage">
        <Image
          ...
          className="premium-product-gallery__image"
          priority={active === 0}
        />
      </div>
      {safeImages.length > 1 && (
        <div className="premium-product-gallery__thumbs" role="list" aria-label="صور المنتج">
          {safeImages.map((image, index) => (
            <button
              key={`${image.url}-${index}`}
              type="button"
              className={`premium-product-gallery__thumb ${index === active ? 'is-active' : ''}`}
