...
export default function OrderForm({ product, plans }: { product: any; plans: Plan[] }) {
  ...
  if (done) {
    return (
      <div className="mt-8 rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-6">
        <h2 className="text-xl font-black text-emerald-500">تم استلام طلبك بنجاح</h2>
        <p className="mt-2 text-sm text-slate-500">سنتواصل معك لمتابعة الطلب وتأكيد التفاصيل.</p>
        {botUsername ? (
          <a
            href={`https://t.me/${botUsername}`}
            target="_blank"
            rel="noreferrer"
            className="mt-5 inline-flex rounded-2xl bg-sky-400 px-5 py-3 text-sm font-black text-slate-950"
          >
            افتح تيليجرام للمتابعة الأسرع
          </a>
        ) : null}
      </div>
    );
  ...
  return (
    <div className="mt-8 rounded-3xl border border-slate-200 p-5 dark:border-white/10">
      <div className="flex items-center justify-between gap-3">
        ...
        {botUsername ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-full border border-sky-400/30 px-3 py-1.5 text-xs font-bold text-sky-600 dark:text-sky-300"
          >
            تواصل عبر تيليجرام
          </a>
        ) : null}
      ...
      {product.installment_enabled && plans.length > 0 && (
        <div className="mt-5">
          ...
          {plan && (
            <div className="mt-4 rounded-2xl bg-sky-500/10 p-4 text-sm">
              <p>
                الدفعة الأولى:{' '}
                <b>
                  {money(
                    plan.first_payment_type === 'percentage'
                      ? Number(product.price_syp || 0) * Number(plan.first_payment_value) / 100
              ...
              <p className="mt-1">
                ...
                <b>
                  {money(
                    plan.monthly_amount ??
                      ((Number(plan.total_price ?? product.price_syp) -
                        Number(
                          plan.first_payment_type === 'percentage'
                            ? Number(product.price_syp || 0) * Number(plan.first_payment_value) / 100
                            : plan.first_payment_value,
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <input
          ...
          className="w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 outline-none focus:border-sky-400 dark:border-white/10"
        />
        <input
          required
          value={phone}
        ...
        <button
          disabled={loading}
          className="w-full rounded-2xl bg-sky-400 px-6 py-4 font-black text-slate-950 transition hover:bg-sky-300 disabled:opacity-50"
        >
