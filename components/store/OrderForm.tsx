...
export default function OrderForm({ product, plans }: { product: any; plans: Plan[] }) {
  const [planId, setPlanId] = useState(''); const [name, setName] = useState(''); const [phone, setPhone] = useState(''); const [address, setAddress] = useState(''); const [notes, setNotes] = useState(''); const [loading, setLoading] = useState(false); const [done, setDone] = useState(false); const [error, setError] = useState('');
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId: product.id, installmentPlanId: plan?.id ?? null, name: name.trim(), phone: phone.trim(), address: address.trim(), notes: notes.trim(), chatToken }) });
