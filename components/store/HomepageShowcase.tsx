...
function currentSyp(priceUsd: unknown, storedSyp: unknown, rateValue: unknown) {
  ...
  // Stored SYP is the authoritative fallback for existing inventory. Only recalculate
  // when the configured rate looks like a real SYP/USD value rather than a placeholder.
...
function ProductCard({ product, exchangeRate, index }: { product: Product; exchangeRate: unknown; index: number }) {
  ...
  return (
    <Link
      ...
      className="lp-home-product-card group"
      prefetch={index < 2}
    ...
    >
      <div className="lp-home-product-media">
        {image?.url ? (
          <img
            ...
            className="lp-home-product-image"
      ...
      <div className="lp-home-product-info">
        ...
        <h3>{product.name}</h3>
        <div className="lp-home-spec-row">
          <span>جودة موثوقة</span>
          {product.installment_enabled ? <span>تقسيط</span> : <span>ضمان سنة</span>}
        </div>
        <div className="lp-home-product-bottom">
          <div>
            <strong>{priceSyp ? `${priceSyp.toLocaleString('ar-SY')} ل.س` : 'السعر عند الطلب'}</strong>
            <small>{moneyUsd(product.price_usd)}</small>
          </div>
          <span className="lp-home-product-arrow"><ArrowLeft size={17} /></span>
        </div>
      </div>
    </Link>
  );
...
export default function HomepageShowcase({ logo, slides, products, brands, exchangeRate, stats }: {
  ...
  useEffect(() => {
    ...
    const sync = () => setIsPointerCoarse(media.matches);
    sync();
  ...
  return <main data-storefront="true" className="lp-home">
    ...
    <header className="lp-home-header" data-home-header>
      <div className="lp-home-container">
        <nav className="lp-home-nav lp-home-glass" aria-label="التنقل الرئيسي">
    ...
    <section className="lp-home-hero" id="home">
      ...
      <div className="lp-home-container">
        <div className="lp-home-hero-grid">
          <div className="lp-home-hero-copy">
            ...
            <div className="lp-home-hero-actions">
              <Link href="/products" className="lp-home-primary-btn">اكتشف الهواتف <ArrowLeft size={18} /></Link>
              <button type="button" className="lp-home-secondary-btn" onClick={openChat}><MessageCircle size={18} /> تحدث مع مستشار</button>
            </div>
          ...
          <div
            ref={sliderRef}
            className="lp-home-slider-shell lp-home-glass"
            tabIndex={0}
            role="region"
            ...
            <div className="lp-home-slider">
              <div className="lp-home-slides" style={{ transform: `translate3d(-${currentSlide * 100}%,0,0)` }}>
                {slideItems.map((slide, index) => (
                  <article className={`lp-home-slide ${index === currentSlide ? 'active' : ''}`} key={`${slide.title}-${index}`} aria-hidden={index !== currentSlide} aria-roledescription="slide" aria-label={`${index + 1} من ${slideItems.length}`}>
                    ...
                    <div className="lp-home-slide-overlay" />
                    <div className="lp-home-slide-content"><span>{slide.tag}</span><h2>{slide.title}</h2><p>{slide.description}</p></div>
                  </article>
                ))}
              </div>
              {slideItems.length > 1 && <>
                <div className="lp-home-slider-controls">
                  <button type="button" onClick={() => { goTo(currentSlide - 1); pause(); }} aria-label="العرض السابق"><ChevronRight size={19} /></button>
                  <button type="button" onClick={() => { goTo(currentSlide + 1); pause(); }} aria-label="العرض التالي"><ChevronLeft size={19} /></button>
                </div>
                <div className="lp-home-dots" aria-label="مؤشر العروض">{slideItems.map((_, index) => <button type="button" key={index} aria-label={`الانتقال إلى العرض ${index + 1}`} aria-current={index === currentSlide} className={index === currentSlide ? 'active' : ''} onClick={() => { goTo(index); pause(); }} />)}</div>
              </>}
            </div>
          </div>
        </div>
    ...
    <section className="lp-home-section" id="brands"><div className="lp-home-container">
      ...
      <div className="lp-home-brand-grid">
        {brands.slice(0, 6).map((brand, index) => <Link href={`/products/brand/${brand.slug}`} prefetch={index < 2} key={brand.id} className="lp-home-brand-card group" aria-label={`ماركة ${brand.name}`}>
          <div className="lp-home-brand-visual">{brand.imageUrl ? <img src={brand.imageUrl} alt={brand.name} loading="lazy" decoding="async" /> : <span>{brand.name}</span>}</div>
          <div className="lp-home-brand-bottom"><strong>{brand.name}</strong><ArrowLeft size={17} /></div>
        </Link>)}
      </div>
    ...
    <section className="lp-home-section lp-home-featured" id="featured"><div className="lp-home-container">
      ...
      <div className="lp-home-products-grid">{products.slice(0, 6).map((product, index) => <ProductCard key={product.id} product={product} exchangeRate={exchangeRate} index={index} />)}</div>
