  ...
  return <header className="sticky top-0 z-40 border-b border-sky-300/10 bg-slate-950/75 backdrop-blur-2xl">
    <div className={`mx-auto flex max-w-7xl items-center justify-between px-4 ${compact ? 'py-3' : 'py-4'} sm:px-6 lg:px-8`}>
      <Link href="/" className="flex min-w-0 items-center gap-3">
        {assets.logo?.url ? <img src={assets.logo.url} alt="Louay Phone" className={`${compact ? 'h-8' : 'h-10'} w-auto max-w-[180px] object-contain`} /> : <div><div className="text-xl font-black tracking-tight text-white">Louay <span className="text-sky-400">Phone</span></div><div className="hidden text-[9px] font-bold tracking-[.22em] text-slate-500 sm:block">PREMIUM SMARTPHONES</div></div>}
      </Link>
