...
export default function ChatWidget() {
  ...
  async function refresh(currentToken = token) {
    ...
    try {
      ...
      if (res.ok) { setMessages(data.messages || []); setTicketId(data.ticketId || ''); }
    } catch {}
  }
  ...
  async function openSession() {
    ...
    try {
      ...
      const res = await fetch('/api/chat/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: saved, name: name.trim() || undefined }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'تعذر فتح المحادثة');
      localStorage.setItem(TOKEN_KEY, data.token);
      setToken(data.token); setTicketId(data.ticketId || ''); setName(data.visitorName || name);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!currentToken) { await openSession(); currentToken = localStorage.getItem(TOKEN_KEY) || ''; }
      if (!currentToken) throw new Error('تعذر فتح المحادثة');
  ...
  useEffect(() => { if (open) void openSession(); }, [open]);
  useEffect(() => { if (!open || !token) return; const timer = window.setInterval(() => void refresh(), 2500); return () => window.clearInterval(timer); }, [open, token]);
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/30 p-0 backdrop-blur-[2px] sm:items-end sm:justify-end sm:p-5" dir="rtl">
      <div className="flex h-[min(680px,100dvh)] w-full max-w-md flex-col overflow-hidden rounded-t-3xl bg-white text-slate-900 shadow-2xl dark:bg-slate-950 dark:text-white sm:h-[680px] sm:rounded-3xl">
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-4 dark:bg-slate-900">
          {!name && !token && <div className="mx-auto mt-10 max-w-xs text-center"><div className="text-4xl">👋</div><h3 className="mt-3 text-xl font-black">أهلًا وسهلًا</h3><p className="mt-2 text-sm text-slate-500">اكتب اسمك لنبدأ المحادثة معك.</p><input value={name} onChange={e => setName(e.target.value)} placeholder="اسمك" className="mt-5 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 caret-sky-500 outline-none focus:border-sky-500 dark:border-white/10 dark:bg-slate-950 dark:text-white" /><button onClick={openSession} disabled={loading || name.trim().length < 2} className="mt-3 w-full rounded-2xl bg-sky-500 px-4 py-3 font-black text-white disabled:opacity-50">{loading ? 'جارٍ البدء...' : 'بدء المحادثة'}</button></div>}
          {(token || name) && <>{messages.length === 0 && <div className="mt-10 text-center text-sm text-slate-400">اكتب رسالتك وسنرد عليك بأسرع وقت.</div>}{messages.map(m => <div key={m.id} className={`mb-3 flex ${m.senderType === 'user' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[82%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-6 ${m.senderType === 'user' ? 'rounded-br-md bg-sky-500 text-white' : 'rounded-bl-md bg-white text-slate-900 shadow-sm dark:bg-slate-800 dark:text-white'}`}>{m.text}</div></div>)}<div ref={bottomRef} /></>}
          {error && <p className="mt-3 rounded-xl bg-red-500/10 p-3 text-sm text-red-500">{error}</p>}
        </div>
        <div className="border-t border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950"><div className="flex gap-2"><input disabled={!token} value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendMessage(); } }} placeholder={token ? 'اكتب رسالتك...' : 'جارٍ فتح المحادثة...'} className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 caret-sky-500 outline-none placeholder:text-slate-400 focus:border-sky-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-white/10 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-500" /><button disabled={!token || !draft.trim() || sending} onClick={sendMessage} className="rounded-2xl bg-sky-500 px-5 font-black text-white disabled:opacity-50">{sending ? '...' : 'إرسال'}</button></div></div>
      </div>
    </div>}
  </>;
}
