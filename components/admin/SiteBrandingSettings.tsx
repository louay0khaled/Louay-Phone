...
type AssetKey = 'logo' | 'fontRegular' | 'fontBold';
type UploadState = { busy: boolean; message: string; ok?: boolean };
const initial: Record<AssetKey, UploadState> = { logo: { busy: false, message: '' }, fontRegular: { busy: false, message: '' }, fontBold: { busy: false, message: '' } };
...
export default function SiteBrandingSettings() {
  ...
  async function upload(key: AssetKey, file: File) {
    ...
    try {
      const prepared = key === 'logo' ? await compressImage(file, 1200, 600) : file;
  ...
  return <section className="glass mt-5 rounded-3xl p-7"><div className="mb-6"><span className="luxury-badge">BRAND SYSTEM</span><h2 className="mt-3 text-xl font-extrabold">هوية الموقع</h2><p className="mt-2 text-sm leading-7 text-slate-400">الشعار والخطوط تُدار هنا. أما الـHero والهواتف المميزة فأصبحت مرتبطة مباشرة بمنتجات المتجر من قسم الواجهة الرئيسية أعلاه.</p></div><div className="grid gap-4 lg:grid-cols-2"><Field keyName="logo" title="شعار المتجر" hint="PNG أو JPG. يفضّل نسخة بخلفية شفافة." accept="image/png,image/jpeg" /><Field keyName="fontRegular" title="الخط العادي" hint="TTF أو OTF. يطبق على نصوص الموقع العادية." accept=".ttf,.otf,font/ttf,font/otf" /><Field keyName="fontBold" title="الخط العريض" hint="TTF أو OTF. يطبق على العناوين والأزرار والأوزان الثقيلة." accept=".ttf,.otf,font/ttf,font/otf" /></div></section>;
}
