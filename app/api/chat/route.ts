...
async function getOrCreateConversation() {
  ...
  if (visitorToken) {
    ...
    if (data) return { db, conversation: data, visitorToken, isNew: false };
  }
  ...
  for (let attempt = 0; attempt < 3 && !conversation; attempt++) {
    ...
    const { data, error } = await db.from('conversations').insert({ visitor_token: visitorToken, ticket_code: code, status: 'open', last_message_at: new Date().toISOString() }).select('id,ticket_code,visitor_name,status,last_message_at').single();
  ...
  return { db, conversation, visitorToken, isNew: true };
}
...
export async function POST(request: Request) {
  try {
    ...
    if (body.name && !conversation.visitor_name) {
      await db.from('conversations').update({ visitor_name: body.name }).eq('id', conversation.id);
    }
