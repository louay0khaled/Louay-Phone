...
const FUNCTION_URL = `${SUPABASE_URL}/functions/v1/telegram-bot-v3`;
...
export async function POST(request: NextRequest) {
  ...
  if (expectedSecret && providedSecret !== expectedSecret) return NextResponse.json({ ok: false, error: 'forbidden' }, { status: 403 });
  try {
