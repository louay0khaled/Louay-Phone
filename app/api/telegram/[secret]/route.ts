...
export async function POST(req: Request, { params }: { params: Promise<{ secret: string }> }) {
  ...
  try {
    ...
    if (!customerId) { const { data: insertedCustomer, error: customerError } = await supabase.from('customers').insert({ name: displayName, phone: String(fromId), telegram_user_id: fromId, telegram_chat_id: chatId, telegram_username: username }).select('id').single(); if (customerError) throw customerError; customerId = insertedCustomer.id; }
