...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: `${product.name}${product.model ? ` ${product.model}` : ''}`, description, alternates: { canonical: `/product/${product.slug}` }, openGraph: { type: 'website', title: `${product.name} | Louay Phone`, description, url: `/product/${product.slug}`, ...(image ? { images: [{ url: image, alt: product.name }] } : {}) } };
}
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [reviewResult, planResult] = await Promise.all([
    ...
    supabase ? supabase.from('installment_plans').select('id,months,first_payment_type,first_payment_value,total_price,monthly_amount').eq('product_id', product.id).eq('is_active', true).order('months', { ascending: true }) : Promise.resolve({ data: [], error: null }),
  ]);
  ...
  return <main>
    ...
    <section className="section section--gray" id="order"><div className="container order-layout"><div><div className="eyebrow">ORDER</div><h2 className="section__title">جاهز للطلب؟</h2><p className="section__lead">أرسل بياناتك، وسنراجع التوفر ونؤكد الطلب معك.</p></div><OrderForm productId={product.id}/></div></section>
    <section className="final-cta"><div className="container"><div className="eyebrow">LOUAY PHONE</div><h2>اختيار أبسط.<br/>قرار أفضل.</h2><div className="actions"><Link href="/products" className="btn btn--dark">العودة للكتالوج</Link><Link href="/#about" className="btn btn--link">تواصل معنا</Link></div></div></section>
  </main>;
}
