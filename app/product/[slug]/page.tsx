...
export default async function ProductDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main dir="rtl" className="min-h-screen bg-white text-slate-950 dark:bg-slate-950 dark:text-white">
    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-10 lg:grid-cols-2">
      <div className="space-y-4"><div className="product-media aspect-square overflow-hidden rounded-[2rem]">{images[0]?.url ? <img src={images[0].url} alt={images[0].alt_text ?? p.name} className="product-image h-full w-full object-contain p-8" /> : <div className="flex h-full items-center justify-center text-slate-400">لا توجد صورة</div>}</div><div className="grid grid-cols-5 gap-3">{images.slice(0, 5).map((i: any) => <div key={i.id} className="product-media aspect-square overflow-hidden rounded-2xl border border-slate-200 dark:border-white/10"><img src={i.url} alt={i.alt_text ?? p.name} className="product-image h-full w-full object-contain p-2" /></div>)}</div></div>
      <div className="pt-2"><p className="font-bold text-sky-500">{p.brands?.name ?? 'Louay Phone'}</p><h1 className="mt-2 text-4xl font-black tracking-tight">{p.name}</h1>{p.model && <p className="mt-2 text-slate-500">{p.model}</p>}<div className="mt-7"><b className="text-3xl">{priceSyp ? priceSyp.toLocaleString('ar-SY') + ' ل.س' : 'السعر عند الطلب'}</b>{p.price_usd && <p className="mt-1 text-sm text-slate-500">${Number(p.price_usd).toLocaleString()}</p>}</div>{p.description && <p className="mt-7 whitespace-pre-line leading-8 text-slate-600 dark:text-slate-300">{p.description}</p>}<div className="mt-8 rounded-3xl border border-slate-200 p-5 dark:border-white/10"><h2 className="font-black">المواصفات</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{Object.entries(specs).map(([k, v]) => <div key={k} className="rounded-xl bg-slate-50 p-3 dark:bg-white/[.04]"><span className="text-xs text-slate-500">{k}</span><p className="mt-1 font-bold">{String(v)}</p></div>)}</div></div><OrderForm product={p} plans={plans} /></div>
    </section>
  </main>;
}
