...
function isPhone(product: Product) { const text = `${product.name} ${product.model ?? ''} ${product.brand ?? ''}`.toLowerCase(); return !blockedTerms.some((term) => text.includes(term)); }
...
function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-card__copy"><h3>{product.name}</h3><p>{product.model ?? 'اختيار مميز من Louay Phone.'}</p><div className="price">{formatUsd(product.price_usd)}{product.installment_enabled ? <small>يتوفر خيار التقسيط</small> : null}</div><div className="product-card__actions"><Link href={`/product/${product.slug}`} className="btn btn--dark">معرفة المزيد</Link><Link href={`/product/${product.slug}#order`} className="btn btn--link">اطلب الآن</Link></div></div>
  </article>;
}
...
export default async function ProductsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const allProducts = (await getActiveProducts(48)).filter(isPhone);
  ...
  return <main>
    ...
    <section className="section section--gray"><div className="container"><header className="section__head"><div className="eyebrow">SMARTPHONE CATALOG</div><h1 className="section__title">كل الهواتف.</h1><p className="section__lead">ابحث ضمن المنتجات الحقيقية الموجودة في المتجر، مع الأسعار والتوفر والصور الحالية.</p></header>
      <div className="catalog-toolbar" aria-label="أدوات البحث والفلترة"><form className="catalog-search" action="/products" method="get"><input name="q" defaultValue={params.q ?? ''} placeholder="ابحث عن هاتف أو ماركة…" aria-label="ابحث عن هاتف أو ماركة" />{selectedBrand && <input type="hidden" name="brand" value={selectedBrand} />}<button className="btn btn--dark" type="submit">بحث</button></form><div className="catalog-brands"><span>الماركات:</span><Link className={`catalog-chip${!selectedBrand ? ' catalog-chip--active' : ''}`} href="/products">الكل</Link>{brands.slice(0, 10).map((brand) => <Link key={brand} className={`catalog-chip${brand === selectedBrand ? ' catalog-chip--active' : ''}`} href={`/products?brand=${encodeURIComponent(brand)}`}>{brand}</Link>)}</div></div>
