...
function clampPage(value: string | string[] | undefined) {
  const n = Number(Array.isArray(value) ? value[0] : value || 1);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : 1;
}

function ProductCard({ product, brand, rate }: { product: any; brand: any; rate: unknown }) {
  ...
  const note = product.specs?.notes;
  const priceSyp = currentSyp(product.price_usd, product.price_syp, rate);
  ...
  return <main data-storefront="true" className="storefront-catalog min-h-screen overflow-x-clip bg-[#020617] text-white luxury-grid">
    ...
    <section className="mx-auto max-w-7xl px-4 pb-20 pt-8 sm:px-6 lg:px-8">
      ...
      {totalPages > 1 && <nav className="mt-10 flex flex-wrap items-center justify-center gap-2" aria-label="صفحات المنتجات">
        ...
        {page < totalPages && <Link href={`/products/brand/${brand.slug}?page=${page + 1}`} className="luxury-button-secondary">التالي</Link>}
      </nav>}
    </section>
  </main>;
}
