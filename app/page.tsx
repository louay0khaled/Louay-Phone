    ...
    <Link href={`/product/${product.slug}`} className="group luxury-card overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-end justify-between gap-3">
          <div><b className="text-base">{product.price_syp ? `${Number(product.price_syp).toLocaleString('ar-SY')} ل.س` : 'السعر عند الطلب'}</b>{product.price_usd && <p className="mt-1 text-xs text-slate-500">${Number(product.price_usd).toLocaleString()}</p>}</div>
    ...
    <main className="min-h-screen overflow-hidden bg-[#020617] text-white luxury-grid">
      ...
      <section className="relative mx-auto grid min-h-[calc(100dvh-72px)] max-w-7xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.08fr_.92fr] lg:px-8 lg:py-20">
        ...
        <div className="relative mx-auto w-full max-w-md lg:max-w-lg">
          ...
          <div className="glass glow relative rounded-[2rem] p-3 sm:p-5">
            <div className="relative flex min-h-[500px] flex-col justify-between overflow-hidden rounded-[1.5rem] border border-sky-300/10 bg-[radial-gradient(circle_at_50%_35%,rgba(14,165,233,.16),transparent_34%),linear-gradient(145deg,#0b1424,#020617)] p-6 sm:min-h-[560px]">
              <div className="flex items-center justify-between text-[10px] font-black tracking-[.25em] text-sky-300"><span>LOUAY PHONE</span><span>2026 COLLECTION</span></div>
              <div className="relative mx-auto flex h-64 w-36 rotate-6 items-center justify-center rounded-[2.4rem] border-[5px] border-slate-600 bg-slate-950 shadow-[0_35px_80px_rgba(0,0,0,.65),0_0_55px_rgba(14,165,233,.13)]"><div className="absolute right-2 top-6 h-20 w-3 rounded-full bg-slate-800" /><div className="h-40 w-2 rounded-full bg-gradient-to-b from-sky-300/50 to-transparent" /></div>
              <div><div className="text-xs font-bold tracking-[.18em] text-slate-500">PREMIUM COLLECTION</div><div className="mt-2 text-2xl font-black sm:text-3xl">أحدث التقنيات،<br />بأسلوب يليق بك.</div><div className="mt-5 h-px bg-gradient-to-l from-sky-400/50 to-transparent" /></div>
            </div>
          </div>
        </div>
      </section>
