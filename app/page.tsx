    ...
    <main className="min-h-screen bg-[#030712] text-white luxury-grid">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#030712]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm text-slate-300 md:flex">
            ...
            <a href="#contact" className="transition hover:text-sky-300">تواصل معنا</a>
          </nav>
      ...
      <section className="mx-auto grid min-h-[78vh] max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[1.1fr_.9fr] lg:px-8">
        <div>
          ...
          <div className="mt-9 flex flex-wrap gap-3"><Link href="/products" className="rounded-2xl bg-sky-400 px-7 py-3.5 font-extrabold text-slate-950 transition hover:bg-sky-300">استكشف الهواتف</Link><a href="#contact" className="rounded-2xl border border-white/10 bg-white/5 px-7 py-3.5 font-bold text-white transition hover:bg-white/10">تواصل معنا</a></div>
        </div>
      ...
      <section id="products" className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        ...
        {error ? <div className="glass rounded-3xl p-10 text-center text-red-300">تعذر تحميل المنتجات حاليًا.</div> : (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {(products ?? []).map((p: any) => {
              const images = [...(p.product_images ?? [])].sort((a: any, b: any) => (a.position ?? 0) - (b.position ?? 0));
              const image = images.find((x: any) => x.is_primary) ?? images[0];
              return <Link key={p.id} href={`/product/${p.slug}`} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/[.03] transition hover:-translate-y-1 hover:border-sky-400/30 hover:bg-white/[.05]">
                <div className="aspect-square bg-slate-900/70">{image?.url ? <img src={image.url} alt={image.alt_text || p.name} className="h-full w-full object-contain p-5 transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-slate-500">لا توجد صورة</div>}</div>
                <div className="p-5"><p className="text-xs font-bold text-sky-300">{p.brands?.name ?? 'Louay Phone'}</p><h3 className="mt-1 font-black">{p.name}</h3><div className="mt-4 flex items-end justify-between gap-2"><div><b>{p.price_syp ? Number(p.price_syp).toLocaleString('ar-SY') + ' ل.س' : 'السعر عند الطلب'}</b>{p.price_usd && <p className="text-xs text-slate-500">${Number(p.price_usd).toLocaleString()}</p>}</div>{p.installment_enabled && <span className="rounded-full bg-sky-400/10 px-2 py-1 text-[11px] font-bold text-sky-300">تقسيط</span>}</div></div>
              </Link>;
            })}
          </div>
        )}
