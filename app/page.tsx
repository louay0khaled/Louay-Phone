...
function StoreImage({ src, alt, className, priority = false, sizes = '(max-width: 768px) 100vw, 70vw' }: { src: string; alt: string; className?: string; priority?: boolean; sizes?: string }) {
  if (isSupabaseImage(src)) return <Image src={src} alt={alt} width={1200} height={1200} className={className} priority={priority} sizes={sizes} />;
  return <img src={src} alt={alt} className={className} loading={priority ? 'eager' : 'lazy'} fetchPriority={priority ? 'high' : 'auto'} decoding="async" />;
}
...
function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div>
      ...
      <div className="product-card__image">{src ? <StoreImage src={src} alt={product.images[0]?.alt_text ?? product.name} sizes="(max-width: 700px) 100vw, 33vw" /> : <div className="product-card__placeholder">الصورة قيد الإضافة</div>}</div>
    </div>
