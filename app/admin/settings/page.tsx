...
async function saveExchangeRate(formData: FormData) {
  ...
  const { error } = await supabase.from('settings').upsert(
    { key: 'exchange_rate', value: { usd_to_syp: rate }, updated_at: new Date().toISOString() },
    { onConflict: 'key' },
  );
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const currentRate = typeof data?.value === 'object' && data?.value && 'usd_to_syp' in data.value
    ? String((data.value as { usd_to_syp?: number }).usd_to_syp ?? '')
    : '';
  ...
  return <main className="min-h-screen bg-[#030712] text-white luxury-grid"><div className="mx-auto max-w-4xl px-5 py-10 lg:px-8">
    ...
    <section className="glass mt-8 rounded-3xl p-7"><h2 className="text-xl font-extrabold">سعر الصرف اليومي</h2><p className="mt-2 text-sm leading-7 text-slate-400">أدخل قيمة الدولار بالليرة السورية. يقبل النظام الأرقام العربية والفاصلة والنقطة، ويستخدم هذا الرقم لتحديث أسعار الهواتف المعروضة بالليرة.</p><form action={saveExchangeRate}><label className="mt-6 block text-sm font-semibold">1 دولار أمريكي =</label><input name="rate" defaultValue={currentRate} required inputMode="decimal" dir="ltr" className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 outline-none focus:border-sky-400" placeholder="مثال: 13400" /><button type="submit" className="mt-5 rounded-xl bg-sky-400 px-6 py-3 font-extrabold text-slate-950">حفظ سعر الصرف</button></form>{params.saved === '1' && <p className="mt-4 text-sm text-emerald-300">تم حفظ سعر الصرف في Supabase بنجاح.</p>}{params.error === 'rate' && <p className="mt-4 text-sm text-red-300">القيمة غير صالحة. أدخل رقمًا موجبًا مثل 13400 أو ١٣٤٠٠.</p>}{params.error === 'save' && <p className="mt-4 text-sm text-red-300">تعذر حفظ سعر الصرف في قاعدة البيانات.</p>}</section>
