...
export default function AdminProductMedia({ params }: { params: Promise<{ productId: string }> }) {
  ...
  async function upload(file: File) {
    ...
    await saveImageUrl(publicData.publicUrl, productName, false);
  }
  ...
  async function uploadBatch(files: File[]) {
    ...
    try {
      // Serialize uploads so position/primary assignment remains deterministic.
      for (const file of files) await upload(file);
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : 'تعذر رفع إحدى الصور.');
  ...
  async function saveImageUrl(imageUrl: string, altText = productName, manageBusy = true) {
    const clean = imageUrl.trim();
    if (!/^https:\/\//i.test(clean)) { setError('رابط الصورة يجب أن يبدأ بـ https://'); return; }
    if (clean.length > 2000) { setError('رابط الصورة طويل جدًا.'); return; }
    ...
    try {
      ...
      if (insertError || !row) throw new Error(insertError?.message ?? 'تعذر حفظ رابط الصورة.');
      setImages((current) => [...current, row as ImageRow]);
      setUrl('');
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'تعذر حفظ رابط الصورة.');
    } finally {
  ...
  async function moveImage(index: number, direction: -1 | 1) {
    ...
    if (target < 0 || target >= sorted.length || busy) return;
